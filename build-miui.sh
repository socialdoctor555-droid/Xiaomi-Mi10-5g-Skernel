...
# ---------------------------------------------------------------------------
# Known-reject auto-resolution.
#
# ReSukiSU's manual-hook setup shifts/touches a couple of files the susfs
# patch also targets, so these two hunks reliably reject even though the
# change they carry is small and purely additive. Rather than relying on the
# patch's line numbers (which drift as ReSukiSU changes), we locate stable
# text anchors and apply the same change directly. Anything outside these
# two known files still falls through to the fatal diagnostic dump below.
# ---------------------------------------------------------------------------

if [ -f fs/proc/task_mmu.c.rej ]; then
    echo "-- Auto-resolving fs/proc/task_mmu.c.rej (missing susfs_def.h include)..."
    if grep -q "susfs_def.h" fs/proc/task_mmu.c; then
        echo "   -> already present, skipping."
    elif grep -q '^#include <linux/mm_inline.h>$' fs/proc/task_mmu.c; then
        sed -i '/^#include <linux\/mm_inline.h>$/a\
#ifdef CONFIG_KSU_SUSFS_SUS_KSTAT\
#include <linux/susfs_def.h>\
#endif' fs/proc/task_mmu.c
        echo "   -> inserted susfs_def.h include after mm_inline.h include."
    else
        echo "   -> Fatal: anchor '#include <linux/mm_inline.h>' not found in fs/proc/task_mmu.c."
        exit 1
    fi
...
if [ -f include/linux/sched.h.rej ]; then
    echo "-- Auto-resolving include/linux/sched.h.rej (ANDROID_KABI slot 8 + susfs fields)..."
    python3 - "include/linux/sched.h" << 'PYEOF'
import re, sys
...
if any("susfs_last_fake_mnt_id" in l for l in lines):
    print("   -> already present, skipping.")
    sys.exit(0)

anchor = "New fields for task_struct should be added above here"
anchor_idx = next((i for i, l in enumerate(lines) if anchor in l), None)
if anchor_idx is None:
    print("   -> Fatal: anchor comment not found in include/linux/sched.h.")
    sys.exit(1)

reserve8_idx = None
for i in range(anchor_idx, max(anchor_idx - 40, -1), -1):
    if re.search(r'^\s*ANDROID_KABI_RESERVE\(8\);\s*$', lines[i]):
        reserve8_idx = i
        break
if reserve8_idx is None:
    print("   -> Fatal: 'ANDROID_KABI_RESERVE(8);' not found near the task_struct anchor.")
    sys.exit(1)

indent = re.match(r'^(\s*)', lines[reserve8_idx]).group(1)
lines[reserve8_idx] = (
    "#ifdef CONFIG_KSU_SUSFS\n"
    f"{indent}ANDROID_KABI_USE(8, u64 susfs_last_fake_mnt_id);\n"
    "#else\n"
    f"{indent}ANDROID_KABI_RESERVE(8);\n"
    "#endif\n"
)
print("   -> rewired ANDROID_KABI slot 8 for CONFIG_KSU_SUSFS.")

# Line count shifted after the rewrite above; re-locate the anchor and find
# the 'randomized_struct_fields_end' that closes this same struct.
anchor_idx = next((i for i, l in enumerate(lines) if anchor in l), None)
end_idx = None
for i in range(anchor_idx, min(anchor_idx + 20, len(lines))):
    if re.search(r'^\s*randomized_struct_fields_end\s*$', lines[i]):
        end_idx = i
        break
if end_idx is None:
    print("   -> Fatal: 'randomized_struct_fields_end' not found after the task_struct anchor.")
    sys.exit(1)
...
with open(path, "w") as f:
    f.writelines(lines)
print("   -> inserted susfs_task_state / susfs_last_fake_mnt_id fields.")
PYEOF
    py_status=$?
    if [ "$py_status" -ne 0 ]; then
        echo "-- Fatal: automatic include/linux/sched.h reject resolution failed."
        exit 1
    fi
    rm -f include/linux/sched.h.rej include/linux/sched.h.orig
fi

rej_files=$(find . -name "*.rej")
if [ -n "$rej_files" ]; then
    echo "-- Fatal: the susfs kernel patch left unresolved rejects outside the"
    echo "   known auto-fixable set (fs/proc/task_mmu.c, include/linux/sched.h)."
    echo "   This usually happens because ReSukiSU's manual-hook setup already"
    echo "   hand-modified files that the patch also expects to touch."
    echo ""
