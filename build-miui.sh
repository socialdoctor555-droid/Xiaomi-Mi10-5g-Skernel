...
if [ -f fs/proc/task_mmu.c.rej ]; then
    ...
    elif grep -q '^#include <linux/pkeys.h>$' fs/proc/task_mmu.c; then
        sed -i '/^#include <linux\/pkeys.h>$/a\
#ifdef CONFIG_KSU_SUSFS_SUS_KSTAT\
#include <linux/susfs_def.h>\
#endif' fs/proc/task_mmu.c
        echo "   -> inserted susfs_def.h include after pkeys.h include."
    else
        echo "   -> Fatal: anchor '#include <linux/pkeys.h>' not found in fs/proc/task_mmu.c."
        exit 1
    fi
